        ...
        if (!TextUtils.isEmpty(response)){
            try {
                ...
                for (int i = 0; i < allCounties.length(); i++){
                    JSONObject cointyObject = allCounties.getJSONObject(i);
                    County county = new County();
                    county.setCountyName(cointyObject.getString("name"));
                    county.setWeatherId(cointyObject.getString("weather_id"));
